...
export const ItemListContainer = props => {
    const [productosListado, setProductos] = useState([]);
    ...
    return (<> {
        (productosListado.length === 0 ? (<div className={styles.root}><CircularProgress /></div>) : (
            <Grid container>
                <Grid item xs={12}>
                    <Grid container justify="center" >
                        <ItemList productosListado={productosListado} />
                    </Grid>
                </Grid>
            </Grid>
        ))}
